<?php
  ...
  //register car
		...
		if(isset($_POST['update_car']))
		{
            ...
            $query="UPDATE  crms_cars SET car_regno = ?, car_name = ?, car_price =?, car_type = ?, car_features = ?, exterior_img = ?, interior_img = ?, rear_img = ?, front_img =? car_id = ? WHERE car_cat_id =?";
            $stmt = $mysqli->prepare($query);
            $rc=$stmt->bind_param('sssssssssi', $car_regno, $car_name, $car_price, $car_type, $car_features, $exterior_img, $interior_img, $rear_img, $front_img, $car_id);
            $stmt->execute();
